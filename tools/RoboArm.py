...
class RoboArm(ToolPane):
    ...
    def __init__(self, parent, port=None):
        ...
        temp = wx.StaticBox(self, -1, 'RoboArm manipulation')
        ...
        for i in range(self.parent.project.count):
            ...
            temp.position = wx.Slider(temp,
                                      i, 512, 0,
                                      self.parent.project.resolution[i]-1,
            ...
            if (count == 3):
                self.servos[-2].Bind(wx.EVT_SCROLL, self.on_slider2_scroll)
            if (count == 5):
                self.servos[-2].Bind(wx.EVT_SCROLL, self.on_slider4_scroll)
    ...
    def updatePose(self, e=None):
        ...
        if self.curpose != "":
            ...
            if (e.GetId() == 1 or e.GetId() == 3):
                self.parent.project.poses[self.curpose][e.GetId()+1] = 1023 - e.GetInt()
            ...
            if self.live and self.servos[e.GetId()].enable.IsChecked():   # live update   
                ...
                if (e.GetId() == 1 or e.GetId() == 3):
                    ...
                    self.port.setReg(e.GetId()+2, P_GOAL_POSITION_L, [pos1%256,pos1>>8])
                else:
                    self.port.setReg(e.GetId()+1, P_GOAL_POSITION_L, [pos%256,pos>>8])
    ...
    def on_slider2_scroll(self, event):
        value = self.servos[1].position.GetValue()
        self.servos[2].position.SetValue(1023 - value)

    def on_slider4_scroll(self, event):
        value = self.servos[3].position.GetValue()
        self.servos[4].position.SetValue(1023 - value)
