    ...
    def doTran(self, e=None):
        ...
        if e.IsSelection():
            if self.curseq != "":
                ...
                v = str(e.GetString())
                print("v = ",v)
                self.tranPose.SetValue(v[0:v.find(",")]) #Pose name
                print("self.tranPose.SetValue(v[0:v.find(",")]) = ",self.tranPose.SetValue(v[0:v.find(",")]))
                self.tranTime.SetValue(int(v[v.find(",")+1:])) #Transition time
                print("self.tranTime.SetValue(int(v[v.find(",")+1:])) = ",self.tranTime.SetValue(int(v[v.find(",")+1:])))
                self.parent.project.save = True 
